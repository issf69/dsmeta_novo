...
function PluginInspect(options = {}) {
  ...
  function configureServer(server) {
    ...
    server.middlewares.use("/__inspect", sirv__default(DIR_CLIENT, {
      single: true,
      dev: true
    }));
...
`);
  ...
  async function generateBuild() {
    ...
    await fs__default.mkdir(targetDir, { recursive: true });
    await fs__default.copy(DIR_CLIENT, targetDir, { overwrite: true });
    await fs__default.writeFile(
      path.join(targetDir, "index.html"),
      (await fs__default.readFile(path.join(targetDir, "index.html"), "utf-8")).replace(
        'data-vite-inspect-mode="DEV"',
        'data-vite-inspect-mode="BUILD"'
      )
    );
    await fs__default.rm(reportsDir, {
      recursive: true,
      force: true
    });
